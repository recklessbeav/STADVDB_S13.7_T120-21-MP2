...
app.post('/dice', (req, res) => {
    ...
    db.query(query2, (err, result2) => {
        ...
        var dice = req.body.dice_table;
        var month = req.body.dice_date;
        if( dice != "None" ){
            var query = `SELECT
                            continent,
                            country,
...
app.post('/slice', (req, res) => {
    ...
    db.query(query2, (err, result2) => {
        ...
        var slice = req.body.slice_table;
        if( slice != "None" ){
            var query = `SELECT
                            month,
                            sum(`+ slice + `) ` + slice + `
                        ...
                        JOIN
                            case_table c ON f.case_id = c.case_id
                        JOIN
                            date_table d ON f.date_id = d.date_id
                        GROUP BY
                            month`;
